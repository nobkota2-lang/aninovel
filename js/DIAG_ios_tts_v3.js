...
(function () {
  ...
  function ui () {
    ...
    mk('状態を調べる', probe);
    mk('ソースを作って再生', buildAndPlay);
    mk('1文だけ再生', tryOne);
    mk('読み上げ開始', function () {
      L = []; P('=== 読み上げを開始します ===');
      P('AniNovel 本来の再生経路です。[TTS] のログがここに出ます。');
      try { startTTS(); } catch (e) { P('startTTS が例外: ' + e.message); }
      setTimeout(function () {
        P('--- 5秒後 ---');
        P('_audioSourcesJa: ' + Object.keys(window._audioSourcesJa || {}).length + ' 件');
        P('_preAudioDisabled: ' + window._preAudioDisabled);
        try { stopTTS(); P('停止しました'); } catch (e) {}
      }, 5000);
    });
  ...
  // iPhone ではコンソールが見られないので、console の出力をパネルへ流す。
  // AniNovel が出す [TTS] 再生開始 ... type=idb / type=server が見えれば
  // 事前生成音声が使われているかどうかが確定する。
  function hookConsole () {
    ['log', 'info', 'warn', 'error'].forEach(function (k) {
      var orig = console[k].bind(console);
      console[k] = function () {
        try {
          var s = [].slice.call(arguments).map(function (a) {
            if (typeof a === 'string') return a;
            try { return JSON.stringify(a); } catch (e) { return String(a); }
          }).join(' ');
          if (/\[TTS\]|\[Audio\]|\[Flow\]|事前生成|再生開始/.test(s)) P('» ' + s.slice(0, 200));
        } catch (e) {}
  ...
  // 本来 startTTS が行う「音声ソースの構築」を明示的に走らせてから鳴らす。
  // 状態を調べるだけでは構築されないので、0件でも異常とは限らない。
  function buildAndPlay () {
    ...
    if (!items.length) { P('ブロックがありません'); return; }
    P('構築前 _audioSourcesJa: ' + Object.keys(window._audioSourcesJa || {}).length + ' 件');
    var t0 = Date.now();
    _loadAudioManifest().then(_buildAudioSources).then(function () {
      P('構築完了 (' + (Date.now() - t0) + 'ms)');
      P('構築後 _audioSourcesJa: ' + Object.keys(window._audioSourcesJa || {}).length + ' 件');
      var it = items[0];
      var has = false;
      try { has = _hasPreAudio(it.id); } catch (e) { P('_hasPreAudio 例外: ' + e.message); }
      P('先頭ブロックの事前音声: ' + has);
      if (has) {
        var s = window._audioSourcesJa[it.id];
        P('  ソース種別: ' + (s && s.type));
      }
      P('再生します…');
      var t1 = Date.now();
      _speakItem(it, function () { P('onEnd (' + (Date.now() - t1) + 'ms)'); });
      setTimeout(function () {
        P('--- 3秒後 ---');
        P('_preAudioCtrl: ' + (window._preAudioCtrl ? 'あり' : 'なし'));
        var a = document.querySelector('audio');
        if (a) {
          P('audio: readyState=' + a.readyState + ' paused=' + a.paused
            + ' t=' + a.currentTime.toFixed(2) + ' error=' + (a.error ? a.error.code : 'なし'));
          P('  src: ' + String(a.src).slice(0, 70));
        } else P('audio 要素なし → Web Speech 経路に入っています');
      }, 3000);
    }).catch(function (e) {
      P('構築に失敗: ' + (e && e.message), 'ng');
      P('→ ここで失敗すると startTTS の .then が実行されず、');
      P('  音声ソースが空のまま Web Speech に落ちます。');
    });
  }
